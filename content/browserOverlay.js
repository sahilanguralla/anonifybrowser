...
AnonifyChrome.BrowserOverlay = {
	 ...
	 */
	userPrefs : Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.anonifybrowser."),
	prompts : Components.classes["@mozilla.org/embedcomp/prompt-service;1"].getService(Components.interfaces.nsIPromptService),
	generalPrefs : Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("general."),
	...
	firstRun : function() {
		try {
			...
			let currentUA = AnonifyChrome.BrowserOverlay.userPrefs.getCharPref("currentUA");
			let enabled = AnonifyChrome.BrowserOverlay.userPrefs.getBoolPref("enabled");
			let osstring = AnonifyChrome.BrowserOverlay.userPrefs.getCharPref("osstring");
			let toggled = AnonifyChrome.BrowserOverlay.userPrefs.getBoolPref("toggled");
			...
			AnonifyChrome.BrowserOverlay.userPrefs.setCharPref("currentUA", "default;,;Default");
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("enabled", false);
			AnonifyChrome.BrowserOverlay.userPrefs.setCharPref("osstring",OSString);
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("toggled",true);
			let UAStringList = [
				["default"],
				["Default","default"],
				["end"],
			...
			AnonifyChrome.BrowserOverlay.userPrefs.setCharPref("uastring",UAString);
		}
	},
	...
	init : function(){
		try{
			let currentOSUA = AnonifyChrome.BrowserOverlay.userPrefs.getCharPref("currentUA");
		...
		let OSString = AnonifyChrome.BrowserOverlay.userPrefs.getCharPref("osstring").split(";,;");
	...
	onOSSelect : function(value, selected, toggled){
		...
		let OSVerList_menupopup = OSVerList.firstChild;
		let OSVerString = AnonifyChrome.BrowserOverlay.userPrefs.getCharPref("uastring").split(";,;");
		panel.hidePopup();
		
		//cleaning OS Version list
		while (OSVerList_menupopup.firstChild) {
		    OSVerList_menupopup.removeChild(OSVerList_menupopup.firstChild);
		}
		...
		AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("toggled", toggled?toggled:false);
	},
	...
	toggleState : function(){
		...
		if(AnonifyChrome.BrowserOverlay.userPrefs.getBoolPref("toggled")){
			let enabled = AnonifyChrome.BrowserOverlay.userPrefs.getBoolPref("enabled");
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("enabled", !enabled);
		}
		else
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("toggled",true);
	},
	...
	onOSVerSelect : function(value){
		...
		if(os != "default")
			AnonifyChrome.BrowserOverlay.userPrefs.setCharPref("currentUA", os+";,;"+value[0]);
		AnonifyChrome.BrowserOverlay.applyUA(value[1]);
		if(os == "default")
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("enabled", false);
		else
			AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("enabled", true);

		AnonifyChrome.BrowserOverlay.userPrefs.setBoolPref("toggled", false);
	},
	...
	applyUA : function(value){
		if(value!="default")
			AnonifyChrome.BrowserOverlay.generalPrefs.setCharPref("useragent.override", value);
		else
			AnonifyChrome.BrowserOverlay.generalPrefs.clearUserPref("useragent.override");
	},
...
window.addEventListener("load", function() {
	AnonifyChrome.BrowserOverlay.firstRun();
