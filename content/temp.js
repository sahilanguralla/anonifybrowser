 ...
 */
let userPrefs = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.tbthello.");
...
TBTChrome.BrowserOverlay = {
	...
	init : function(){
		...
		let OSList = document.getElementById("tbthello-toolbar-menu-os-menulist").firstChild;
		...
		for(let i=0; i < OSString.length; i++){
			...
			if(OSString[i][0] == currentOS){
				document.getElementById("tbthello-toolbar-menu-os-menulist").selectedIndex = i;
			}
		}

		TBTChrome.BrowserOverlay.onOSSelect(currentOS, currentUA);
		
	},

	onOSSelect : function(value, selected){
		let OSVerList = document.getElementById("tbthello-toolbar-menu-osver-menulist").firstChild;
		OSVerList.innerHTML="";
		...
		for(let i=0, j=0; i < OSVerString.length; i++){
			if(start){
				...
				let menuitem = document.createElement("menuitem");
				menuitem = OSVerList.appendChild(menuitem,OSVerList.firstChild);
				menuitem.setAttribute("label", OSVerString[i][0]);
				menuitem.setAttribute("value", OSVerString[i][1]);

				if((selected && OSVerString[i][0] == selected) || (!selected && !j)){
					document.getElementById("tbthello-toolbar-menu-osver-menulist").selectedIndex = j;
				}
				j++;
			}
	...
	onOSVerSelect : function(value){
		...
		if(value!="default")
			prefs.setCharPref("useragent.override", value);
		else
			prefs.clearUserPref("useragent.override");
	}
};

TBTChrome.PrefObserver = {
  register: function() {
    ...
    this.branch = prefService.getBranch("extensions.tbthello.");
  ...
  observe: function(aSubject, aTopic, aData) {
    ...
    switch (aData) {
      case "enabled":
        userPrefs.setCharPref("test","enabled");
        break;
      case "currentUA":
        break;
      case "firstrun":
...
TBTChrome.PrefObserver.register();

window.addEventListener("load", function() { TBTChrome.BrowserOverlay.init(); }, false);